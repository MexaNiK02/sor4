...
В вопрос можно добавить до 4 картинок. Картинку можно загрузить с компьютера или вставить ссылку и нажать Enter. Если добавлена ссылка, сервер скачивает изображение и дальше использует локальную копию.

На Windows у PHP часто не настроены корневые SSL-сертификаты для cURL. Поэтому для локальной разработки в `.env.example` стоит `LIVEQUIZ_IMAGE_VERIFY_SSL=false`, чтобы скачивание картинок по HTTPS не падало с `cURL error 60`. На VPS/Linux с правильно настроенным PHP можно поставить `LIVEQUIZ_IMAGE_VERIFY_SSL=true`.
...
- `public/uploads/question-images` - пользовательские картинки вопросов, создаются при загрузке из интерфейса.
